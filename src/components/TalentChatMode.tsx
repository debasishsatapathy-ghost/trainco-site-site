...
export function TalentChatMode({ onSend, sessionReady = true, sections }: TalentChatModeProps) {
  ...
  useEffect(() => {
    ...
    const hideAvatarLayers = () => {
      ...
      // Restore React container opacity — deactivateVisualInversion may leave
      // the framer-motion wrapper at opacity: 0 (its `initial` value) because
      // the completed animation's inline style was stripped.
      const rootEl = document.getElementById("root");
      if (rootEl) rootEl.style.opacity = "1";
      const appWrapper = rootEl?.querySelector('[data-testid="base-layout"]')
        ?.closest('[class*="w-screen"]') as HTMLElement | null;
    ...
    return () => {
      ...
      if (typeof fw.showBgLayer === "function") {
        (fw.showBgLayer as () => void)();
      }
      if (typeof fw.setAvatarVolume === "function") {
        (fw.setAvatarVolume as (v: number) => void)(1);
      }
      if (typeof fw.setAvatarVideoMuted === "function") {
        (fw.setAvatarVideoMuted as (v: boolean) => void)(false);
      }
      if (typeof fw.applyVisualInversion === "function") {
        const defaultCfg = typeof fw.getDefaultVisualEffectsConfig === "function"
          ? fw.getDefaultVisualEffectsConfig()
          : null;
        (fw.applyVisualInversion as (cfg: Record<string, unknown>) => void)(
          defaultCfg ?? {
            imageOpacity: 0.6,
            imageInvert: false,
            backgroundOpacity: 0.1,
            backgroundImageOpacity: 0.5,
            backgroundImageInvert: false,
          },
        );
      }
    };
