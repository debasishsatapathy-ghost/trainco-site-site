...
export function MyLearningSheet({ candidateId, initialPhase = "my-learning", onBack, onClose }: MyLearningSheetProps) {
  ...
  useEffect(() => {
    ...
    // Force re-fetch with cleared flag to get "before" data (73%)
    if (candidateId) {
      void fetchSkills("ai-engineer");
      void fetchMarketRelevance(candidateId);
      void fetchCareerGrowth(candidateId);
    }
  }, []); // Run once on mount
    ...
    useMemo<VoiceAction[]>(() => {
      if (phase === "my-learning") {
        return [
          ...
          { phrases: ["start learning", "begin", "kubernetes"], action: () => { void sendTappedIntent("Create a Learning Plan"); setPhase("plan"); }},
        ];
      }
      if (phase === "plan") {
        return [
          { phrases: ["customize", "customize this plan", "edit plan"], action: () => { void sendTappedIntent("Customize this plan"); setPhase("customize"); }},
          { phrases: ["add to my learning", "add to learning", "save"], action: () => { void sendTappedIntent("Add to my learning"); handleMyLearningClose(); }},
          { phrases: ["start learning", "begin", "start"], action: () => { void sendTappedIntent("Start Learning"); setPhase("lesson-video"); }},
        ];
      }
      if (phase === "customize") {
        return [
          { phrases: ["go back", "back"], action: () => { void sendTappedIntent("Go back"); setPhase("plan"); }},
          { phrases: ["update", "update my plan", "save", "confirm"], action: () => { void sendTappedIntent("Update my plan"); setPhase("plan"); }},
        ];
      }
      if (phase === "lesson-video") {
        return [
          { phrases: ["next lesson", "continue", "next"], action: () => { void sendTappedIntent("Next Lesson"); setPhase("lesson-reading"); }},
        ];
      }
      if (phase === "lesson-reading") {
        return [
          { phrases: ["finish course", "finish", "complete", "done"], action: () => { void sendTappedIntent("Finish Course"); handleFinishCourse(); }},
        ];
      }
      if (phase === "results") {
        return [
          { phrases: ["back to profile", "profile", "dashboard", "close"], action: () => { void sendTappedIntent("Back to Profile"); handleMyLearningClose(); }},
        ];
      }
      return [];
    }, [phase]),
        ...
        <div className="flex flex-col gap-6">
          ...
          <div>
            ...
            <LearningCard
              ...
              onClick={() => {
                void sendTappedIntent("Create a Learning Plan");
                setPhase("plan");
              }}
      ...
      >
        ...
        <div className="px-4 pb-8 mt-4 flex justify-end safe-bottom">
          <button
            type="button"
            onClick={() => {
              void sendTappedIntent("Next Lesson");
              setPhase("lesson-reading");
            }}
        ...
        <div className="px-4 flex flex-col gap-4 pb-4">
          ...
          <div className="flex justify-end safe-bottom pb-6">
            <button
              ...
              disabled={submitting}
              onClick={() => {
                void sendTappedIntent("Finish Course");
                void handleFinishCourse();
              }}
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-wrap gap-2 items-stretch justify-stretch">
                <button
                  type="button"
                  onClick={() => {
                    void sendTappedIntent("Customize this plan");
                ...
                <button
                  type="button"
                  onClick={() => {
                    void sendTappedIntent("Add to my learning");
                    setPhase("my-learning");
                  }}
                  className="min-w-0 flex-1 rounded-[24px] px-4 py-3 flex items-center justify-center no-lightboard bg-[rgba(16,42,40,0.2)]"
                >
                  <span className="text-[#f4f4f5] text-base font-normal leading-6 text-center">
              ...
              <div className="flex justify-end">
                <button
                  type="button"
                  onClick={() => {
                    void sendTappedIntent("Start Learning");
                    setPhase("lesson-video");
                  }}
                  className="inline-flex items-center justify-center gap-2 rounded-[24px] px-4 py-3 no-lightboard bg-[#1dc558] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)]"
                >
                  <span className="text-[#18181b] text-base font-semibold leading-6">
              ...
              <div className="flex flex-wrap gap-2 items-stretch">
                ...
                <div className="min-w-0 flex-1 flex justify-end">
                  <button
                    type="button"
                    onClick={() => {
                      void sendTappedIntent("Update my plan");
                      handleUpdatePlan();
                    }}
                    className="inline-flex items-center justify-center gap-2 rounded-[24px] px-4 py-3 no-lightboard bg-[#1dc558] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)]"
                  >
