...
export function SkillTestFlow({ candidateId: candidateIdProp, onBack, onClose }: SkillTestFlowProps) {
  ...
  useVoiceActions(
    useMemo(() => {
      if (phase === "landing") {
        return [
          { phrases: ["take a test", "test", "take test"], action: () => { void sendTappedIntent("Take a test"); setPhase("prep"); }},
          { phrases: ["create a learning plan", "learning plan", "create plan"], action: () => { void sendTappedIntent("Create a Learning Plan"); setShowLearningPath(true); }},
          { phrases: ["validate", "upload certificate", "certificate"], action: () => { void sendTappedIntent("Validate outside learning"); setPhase("upload"); }},
        ];
      }
      if (phase === "prep") {
        return [
          { phrases: ["start test", "begin", "ready", "start"], action: () => { void sendTappedIntent("Start Test"); setPhase("q0"); }},
        ];
      }
      if (phase === "q0") {
        return [
          { phrases: ["next question", "next", "continue"], action: () => { if (mcqSelected) { void sendTappedIntent("Next Question"); setPhase("q1"); }}},
        ];
      }
      if (phase === "q1") {
        return [
          { phrases: ["next question", "next", "continue"], action: () => { if (tfSelected !== null) { void sendTappedIntent("Next Question"); setPhase("q2"); }}},
        ];
      }
      if (phase === "q2") {
        return [
          { phrases: ["submit", "submit answer", "done"], action: () => { if (essayText.trim() && !submitting) { void sendTappedIntent("Submit Answer"); void handleSubmitEssay(); }}},
        ];
      }
      if (phase === "results") {
        return [
          { phrases: ["back to profile", "profile", "dashboard"], action: () => handleClose(true) },
        ];
      }
      return [];
    }, [phase, mcqSelected, tfSelected, essayText, submitting]),
  );
            ...
            <div className="flex flex-col gap-2.5">
              ...
              <div className="flex flex-wrap items-start justify-end gap-x-2 gap-y-2.5 w-full">
                <button
                  type="button"
                  onClick={() => {
                    void sendTappedIntent("Take a test");
                    setPhase("prep");
                  }}
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex justify-end">
                <button
                  type="button"
                  onClick={() => {
                    void sendTappedIntent("Start Test");
                    setPhase("q0");
                  }}
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex justify-end">
                <button
                  ...
                  onClick={() => {
                    if (mcqSelected) {
                      void sendTappedIntent("Next Question");
                      setPhase("q1");
                    }
                  }}
                  className={
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex justify-end">
                <button
                  ...
                  onClick={() => {
                    if (essayText.trim() && !submitting) {
                      void sendTappedIntent("Submit Answer");
