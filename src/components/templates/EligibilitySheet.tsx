...
export function EligibilitySheet({
  ...
  useEffect(() => {
    ...
    parts.push(
      "Now describe the candidate's eligibility with specific insights: start with their overall fit, " +
      "highlight their key strengths by name, name any gaps they need to close, " +
      "and suggest the logical next step (apply if strong fit, close the gap if stretch or grow-into). " +
      "Keep it to 2–3 sentences. Respond now.",
    );

    const instruction = parts.join(" ");
    // Short delay so the pre-navigation speech ("Let me check your eligibility") finishes
    // before we ask for the insight response. teleAcknowledge actively triggers a new response,
    // unlike informTele which is silent context only.
    const t = setTimeout(() => teleAcknowledge(instruction, { visible: false }), 800);
  ...
  useVoiceActions(
    useMemo(
      () => [
        ...
        // Skill Alignment accordion
        { phrases: ["open skills", "show skills", "expand skills", "skill alignment", "open skill alignment", "show skill alignment"], action: handleOpenSkills },
        { phrases: ["close skills", "hide skills", "collapse skills", "close skill alignment", "hide skill alignment"], action: handleCloseSkills },
        { phrases: ["toggle skills", "skills"], action: handleToggleSkills },
        // Relevant Experience accordion
        { phrases: ["open experience", "show experience", "expand experience", "relevant experience", "open relevant experience", "show relevant experience"], action: handleOpenExp },
        { phrases: ["close experience", "hide experience", "collapse experience", "close relevant experience", "hide relevant experience"], action: handleCloseExp },
        { phrases: ["toggle experience", "experience"], action: handleToggleExp },
        // Certifications accordion
        { phrases: ["open certifications", "show certifications", "expand certifications", "certifications", "open certifications", "show my certifications"], action: handleOpenCert },
        { phrases: ["close certifications", "hide certifications", "collapse certifications", "close certifications", "hide my certifications"], action: handleCloseCert },
        { phrases: ["toggle certifications"], action: handleToggleCert },
      ],
      [handleClose, handleApply, handleCloseGap,
       handleOpenSkills, handleCloseSkills, handleToggleSkills,
       handleOpenExp, handleCloseExp, handleToggleExp,
       handleOpenCert, handleCloseCert, handleToggleCert],
    ),
  );
